...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (interaction.isButton() && customId === 'reportto') {
            ...
            try {
                ...
                const embedKick = new EmbedBuilder()
                    ...
                    .setTitle(`Te dieron timeout de ${interaction.guild.name}`)
                    .setColor('Red')
                    .setDescription(`Porfavor no seas toxico o no **rompas las reglas del servidor**`)
                    .setTimestamp()
                ...
                await user.timeout(900000)
                await message.edit({ embeds: [editEmbed], components: [] })
                return correReply(interaction, "Se sanciono correctamente al usuario (TimeOut)", true)
            } catch (error) {
                console.log(error);
